...
- **Support Assessment Result** — The final semantic result for one fixed claim and independent Support: **Supported** with one current Primary and zero or more Required references, or **Unsupported** after authoritative Full coverage. Delta cannot conclude Unsupported. _Avoid_: Insufficient as a third semantic result
- **Needs Full** — An internal Support-execution transition emitted after complete Delta processing cannot establish Supported. It continues through the remaining current-revision contexts and is neither lifecycle state nor a final Support result.
- **Unresolved Semantic Work** — An application-owned execution outcome for incomplete coverage, manifest, capacity, provider/schema execution, model abstention, or order disagreement. It preserves current Support, blocks destructive action, and does not advance the validation baseline. _Avoid_: Unsupported, model-generated Insufficient
