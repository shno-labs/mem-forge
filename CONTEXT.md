...
- **Source Unit** — The stable provider-neutral identity of one independently addressable provider item inside a configured Source. Within that Source, its Provider Key identifies the item and its Unit Type is immutable classification of the adapter-defined unit boundary.
- **Unit Type** — The adapter-defined classification of a Source Unit, such as `jira_issue`, `confluence_page`, or `teams_window`. It describes the unit boundary and cannot identify a particular Unit without its Source and Provider Key.
- **Source Observation** — The stable identity of one provider-defined content part inside a Source Unit, such as a Jira issue core, comment, or changelog entry. Its Provider Key identifies the part within the Unit and its Observation Type is immutable classification.
- **Unit Title**: The provider's human-facing name of one Source Unit, such as a Jira key, type and summary or a Confluence space and page title, taken only from values present in the provider payload. Every adapter projects it as the Unit's first Source Observation (`unit_identity`); every model reading of the Unit reads it, and it can be selected only as Required Evidence. It is content, not the Source Unit identity that Provider Key and Unit Type define, and not the Memory identity that identity matching decides. _Avoid_: document header
- **Source Observation Revision** — One immutable content version of a Source Observation. Revision-pinned Evidence refers to this exact content rather than to a mutable current body.
- **Source Unit Revision** — An append-only aggregate revision representing an immutable Source Unit snapshot or manifest. It contains no source body; it pins the effective Source Observation Revision set and aggregate semantic, location, membership, and access fingerprints under declared Projection Coverage.
...
- **Reprocess at Current Revision**: An operator-requested Source sync run that processes named Documents from their stored input (the item metadata their Gene discovered, the raw content, and the Artifacts of their committed Source Unit Revision) with the current adapter and compiler, without contacting the provider. Extraction reads every Reading Group and every Support is read over the whole Unit as if it had no usable baseline; the run keeps the sync cursor and infers no removal. It exists for Units that no longer change, such as closed Jira issues, after an adapter or compiler change. _Avoid_: force-resync, rebaseline
...
- **Reading Group** — A revision-scoped coherent reading unit containing one or more Evidence Fragments plus the minimum structural context needed to interpret them: one outermost list, or one Fragment. Claim Extraction and Support Assessment share the same Reading Groups, and every reading also carries the Observation its provider says the read one replies to or follows, and the Unit Title, which is never a Reading Group of its own. It is not one Evidence Unit, one model request, or durable lifecycle state.
...
- **Assessment Scope**: The complete effective current revision, read for one Support work item in a fixed order: every changed Reading Group (added, modified and removed) and the Reading Groups holding the Support's prior Evidence first (the Delta part), then the rest. The work item cannot stop while any Reading Group of the Delta part is unread; after that part it stops reading once it has complete Support. The scope may be delivered through many Assessment Contexts. _Avoid_: Delta mode, Full mode
- **Complete Support**: Selected Evidence completely supports a claim only when every specific the claim states (names of people, systems and things, identifiers, quantities, dates and times, statuses, conditions and scope) appears in that Evidence or follows directly from it. A claim that states any specific the Evidence contradicts or does not contain is not supported, even when the rest matches. Support Assessment and Candidate Admission judge it with the same definition. _Avoid_: topical support, mostly supported
- **Support Assessment Result**: The final semantic result for one fixed claim and independent Support: **Supported** (Complete Support) with one current Primary and zero or more Required references, or **Unsupported** only after the whole reading order has been read under authoritative coverage of every affected object. _Avoid_: Insufficient as a third semantic result
- **Unresolved Semantic Work**: An application-owned Support outcome that commits with the revision, for one of two reasons: partial coverage (an Unknown exact correspondence under partial projection, which makes no model call and takes priority over every other correspondence status) or capacity (one Reading Group that alone exceeds the model's capacity for the Claim, reported with a diagnostic naming the Source Unit and Reading Group). It preserves current Support, blocks automatic destructive action, and does not advance the validation baseline. A transient execution failure (a provider error, a timeout, or a schema or ID failure that remains after the LLM batch runner split the request down to one item and one Reading Group) is not Unresolved: the Source Unit revision is not committed and the next sync retries it. _Avoid_: Unsupported, model-generated Insufficient
- **Change Bundle**: One capacity-safe set of added or modified Reading Groups, plus the old text of removed Reading Groups as removed content, jointly classified against fixed old claims for semantic impact. It is classifier input, not an Evidence Unit or lifecycle state.
- **Candidate Admission**: The check every extracted Candidate passes before Relation: its selected Primary and Required Evidence give the claim Complete Support, including scope, exceptions and qualifying headers, and it does not state the same knowledge as another admitted Candidate of the round. A rejected Candidate is not added and creates no Review; an execution failure leaves the Source Unit revision uncommitted. _Avoid_: Candidate ledger, deduplication against other Units
...
- **Deferred Lifecycle Handle** — The opaque same-process capability returned with a Deferred disposition. Its caller may supply the current run's eligible owner Units and retry it, but cannot inspect or modify prepared Evidence, topology snapshots, authority guards, or attempt accounting. _Avoid_: Prepared Plan payload, retry job
...
- **Cross-Document Relation** — A post-commit annotation between two Memories from different Source Units: `equivalent`, `updates` (the newer statement is ordered by the Evidence time the classifier saw for each Memory, stored with the relation) or `contradicts`. It is attached in search only when the caller can see both Memories, stays valid for their exact content, and never changes either Memory. _Avoid_: Cross-Source Conflict Review
- **Relation Dismissal** — A person's record that one Cross-Document Relation is wrong for the current content of both Memories. It hides that relation until either Memory changes and grants no lifecycle authority.
