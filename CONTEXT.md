...
- **Cross-Document Relation** — A post-commit annotation between two Memories from different Source Units: `equivalent`, `updates` (the newer statement is ordered by Evidence source revision time) or `contradicts`. It is attached in search only when the caller can see both Memories, stays valid for their exact content and Support, and never changes either Memory. _Avoid_: Cross-Source Conflict Review
