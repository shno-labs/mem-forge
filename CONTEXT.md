...
- **Unresolved Semantic Work**: An application-owned execution outcome for incomplete coverage (including an Unknown exact correspondence under partial projection, which makes no model call and takes priority over every other correspondence status), manifest, capacity, provider/schema execution (including a work item that still fails after the LLM batch runner split its request down to that one item and one Reading Group), or model abstention. It preserves current Support, blocks destructive action, and does not advance the validation baseline. _Avoid_: Unsupported, model-generated Insufficient
