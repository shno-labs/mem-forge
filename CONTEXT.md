...
- **Support Assessment Result**: The final semantic result for one fixed claim and independent Support: **Supported** with one current Primary and zero or more Required references, or **Unsupported** only after the whole reading order has been read under authoritative coverage of every affected object. _Avoid_: Insufficient as a third semantic result
...
- **Candidate Admission**: The check every extracted Candidate passes before Relation: its selected Primary and Required Evidence completely support the whole claim, including scope, exceptions, qualifying headers and identifying details such as names and keys, and it does not state the same knowledge as another admitted Candidate of the round. A rejected Candidate is not added and creates no Review; an execution failure leaves the Source Unit revision uncommitted. _Avoid_: Candidate ledger, deduplication against other Units
