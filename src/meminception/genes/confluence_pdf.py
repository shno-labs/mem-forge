...
from meminception.genes.atlassian_auth import get_with_rate_limit_retry
...
async def export_confluence_page_pdf(
    *,
    ...
    if work_dir is None:
        with tempfile.TemporaryDirectory(prefix="meminception-confluence-pdf-") as tmp:
...
def default_chrome_renderer() -> RenderPdf | None:
    ...
    async def render(html_path: Path) -> bytes | None:
        ...
        if _truthy_env("MEMINCEPTION_CHROME_NO_SANDBOX"):
...
def _find_chrome() -> str | None:
    candidates = [
        os.environ.get("MEMINCEPTION_CHROME_PATH"),
