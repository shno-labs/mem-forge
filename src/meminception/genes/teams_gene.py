...
``meminception auth teams`` CLI command. Tokens are cached at
``~/.meminception/tokens/teams.json``.
...
from meminception.pipeline.normalizer_utils import html_to_markdown
...
class _TeamsAPIClient:
    ...
    async def _load_tokens(self) -> tuple[str, str]:
        ...
        Search order:
        1. ~/.meminception/tokens/teams.json (cached)
        2. Chrome cookies (live extraction)
        """
        from meminception.auth.teams_auth import (
            TeamsAuthenticator,
        )
        ...
        if not tokens:
            raise AuthenticationError(
                "No Teams tokens found. Run: meminception auth teams"
            )
        ...
        if not chat_token:
            raise AuthenticationError(
                "Teams Chat API token not found or expired. Run: meminception auth teams"
            )
        if not graph_token:
    ...
    async def validate(self) -> None:
        ...
        try:
            ...
            if resp.status_code == 401:
                raise AuthenticationError(
                    "Teams API returned 401. Run: meminception auth teams"
                )
    ...
    async def _request(
        ...
        from meminception.pipeline.retry import retry_async
