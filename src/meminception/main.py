"""CLI entry point for MemInception."""
...
from meminception.config import AppConfig, load_config
...
def init(ctx):
    ...
    console.print(f"\n[bold green]Initialised MemInception at {base}[/]")
    console.print("\nNext steps:")
    console.print("  1. Configure API keys in config.toml or via environment variables")
    console.print("  2. Run: meminception api     (start the Admin API)")
    console.print("  3. Run: cd admin-ui && npm run dev")
    console.print("  4. Add sources in the admin UI")
    console.print("  5. Run sync from the UI, schedule sync, or run: meminception sync")
    console.print("  6. Run: meminception serve   (start the MCP server)")
...
def sync(ctx, source: str | None):
    ...
    async def _run():
        config: AppConfig = ctx.obj["config"]
        from meminception.storage.database import Database
        from meminception.runtime import build_sync_runtime, run_source_sync
...
def serve(ctx):
    ...
    async def _run():
        config: AppConfig = ctx.obj["config"]
        from meminception.storage.database import Database
        from meminception.server.mcp_server import run_mcp_stdio
...
def memories_stats(ctx):
    ...
    async def _run():
        ...
        from meminception.models import MemoryType
...
def auth_teams(region: str):
    ...
    console.print("\n[bold green]Done! Tokens saved to ~/.meminception/tokens/teams.json[/]")
...
def auth_jira(ctx, base_url: str, browser: str | None, confirm_principal_change: bool):
    ...
    async def _run():
        from meminception.auth.jira_auth import JiraAuthSessionError, JiraAuthSessionService, JiraPrincipalChangedError
...
def auth_status():
    ...
    if not tokens:
        ...
        console.print("Run: [bold]meminception auth teams[/] to authenticate.")
        return
...
def config_show(ctx):
    ...
    console.print("\n[bold]MemInception Configuration[/]\n")
