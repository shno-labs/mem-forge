"""MemInception — Auto-evolutionary agent memory layer for development teams."""
...
DEFAULT_BASE_DIR = Path.home() / ".meminception"
...
class StorageConfig:
    ...
    def resolve(self, base_dir: Path) -> None:
        ...
        if not self.db_path:
            self.db_path = str(base_dir / "db" / "meminception.db")
        if not self.chroma_path:
            self.chroma_path = str(base_dir / "vectors" / "chroma")
        if not self.docs_path:
            self.docs_path = str(base_dir / "documents")
        self.db_path = resolve_path(self.db_path, base_dir / "db" / "meminception.db")
...
class AppConfig:
    ...
    def __post_init__(self) -> None:
        if os.environ.get("MEMINCEPTION_BASE_DIR"):
            self.base_dir = Path(os.environ["MEMINCEPTION_BASE_DIR"]).expanduser()
        if os.environ.get("MEMINCEPTION_STORAGE_DB_PATH"):
            self.storage.db_path = os.environ["MEMINCEPTION_STORAGE_DB_PATH"]
        if os.environ.get("MEMINCEPTION_STORAGE_CHROMA_PATH"):
            self.storage.chroma_path = os.environ["MEMINCEPTION_STORAGE_CHROMA_PATH"]
        if os.environ.get("MEMINCEPTION_STORAGE_DOCS_PATH"):
            self.storage.docs_path = os.environ["MEMINCEPTION_STORAGE_DOCS_PATH"]
        self.storage.resolve(self.base_dir)
        # Environment variable overrides (MEMINCEPTION_* prefix)
        self.llm.enrichment_model = (
            os.environ.get("MEMINCEPTION_ENRICHMENT_MODEL")
            or self.llm.enrichment_model
        )
        self.llm.enrichment_base_url = (
            os.environ.get("MEMINCEPTION_ENRICHMENT_BASE_URL")
            or self.llm.enrichment_base_url
        )
        self.llm.enrichment_api_key = (
            os.environ.get("MEMINCEPTION_ENRICHMENT_API_KEY")
            or self.llm.enrichment_api_key
        )
        self.llm.request_timeout_s = float(
            os.environ.get("MEMINCEPTION_LLM_REQUEST_TIMEOUT_SECONDS")
            or self.llm.request_timeout_s
        )
        self.llm.embedding_model = (
            os.environ.get("MEMINCEPTION_EMBEDDING_MODEL")
            or self.llm.embedding_model
        )
        self.llm.embedding_base_url = (
            os.environ.get("MEMINCEPTION_EMBEDDING_BASE_URL")
            or self.llm.embedding_base_url
        )
        self.llm.embedding_api_key = (
            os.environ.get("MEMINCEPTION_EMBEDDING_API_KEY")
            or self.llm.embedding_api_key
        )
        self.server.admin_api_port = int(
            os.environ.get("MEMINCEPTION_ADMIN_API_PORT")
            or self.server.admin_api_port
        )
        self.server.cors_origins = (
            os.environ.get("MEMINCEPTION_CORS_ORIGINS")
            or self.server.cors_origins
        )
        self.server.jwt_secret = (
            os.environ.get("MEMINCEPTION_JWT_SECRET")
