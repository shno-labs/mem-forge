"""Admin REST API for MemInception.
...
from meminception.auth.jira_auth import (
...
from meminception.genes import GENE_REGISTRY, create_gene, list_available_genes
...
from meminception.memory.lifecycle import normalize_memory_status
...
from meminception.runtime import SyncAlreadyRunningError, SyncService
from meminception.scheduler import SyncScheduler
...
async def _build_memory_store(db: Database, config: AppConfig) -> MemoryStore:
    ...
    from meminception.retrieval.embeddings import get_chroma_collection
...
def create_admin_app(
    ...
    """Create and configure the MemInception Admin API FastAPI application.
    ...
    app = FastAPI(
        title="MemInception Admin API",
        version="0.1.0",
        description="Management API for the MemInception agent memory layer.",
        lifespan=lifespan,
    )
    if db is not None:
        app.state.db = db
    ...
    async def list_entities(
        ...
        async with db.db.execute(query, params) as cursor:
            async for row in cursor:
                d = dict(row)
                from meminception.storage.database import _entity_from_row
    ...
    async def teams_auth_check():
        ...
        from meminception.auth.teams_auth import TeamsAuthenticator

        tokens = TeamsAuthenticator.load_tokens()
        if not tokens:
            return TeamsAuthCheckResponse(
                authenticated=False,
                error="No Teams tokens found. Run: meminception auth teams",
            )
        ...
        if not any(validity.values()):
            return TeamsAuthCheckResponse(
                authenticated=False,
                error="All Teams tokens have expired. Run: meminception auth teams",
            )
    ...
    async def teams_browse(region: str = "emea"):
        ...
        from meminception.auth.teams_auth import TeamsAuthenticator
        from meminception.genes.teams_gene import _TeamsAPIClient

        tokens = TeamsAuthenticator.load_tokens()
        if not tokens:
            raise HTTPException(
                status_code=401,
                detail="No Teams tokens. Run: meminception auth teams",
            )
    ...
    async def delete_source(
        ...
        from meminception.storage.document_store import LocalDocumentStore
    ...
    async def submit_agent_session_window_summary(
        ...
        from meminception.agent_sessions import submit_agent_session_window
    ...
    async def record_agent_hook_receipt(
        ...
        from meminception.agent_sessions import submit_agent_hook_receipt as record_hook_receipt
    ...
    async def build_hook_context(
        ...
        from meminception.agent_hooks import (
