...
def _write_inbox_package(
    *,
    ...
    """Write the legacy/direct local inbox only when no artifact store exists.

    Server intake owns one canonical artifact URI.  Creating a second mutable
    inbox copy made dedupe, rollback, and Source deletion impossible to close
    transactionally across both representations.
    """
...
async def submit_local_markdown_document(
    *,
    ...
    if vault_id.strip() != configured_vault:
        raise ValueError(f"vault_id {vault_id!r} does not match the source's configured vault_id {configured_vault!r}")
...
async def submit_github_repo_document(
    *,
    ...
    if _canonical_repo_url(repo_url) != _canonical_repo_url(configured_repo):
        raise ValueError(f"repo_url {repo_url!r} does not match the source's configured repo_url {configured_repo!r}")
    if not isinstance(markdown_body, str):
        raise ValueError("markdown_body is required")
    ...
    if document_store is None:
        ...
        if not package_path.exists() and _github_package_count(inbox, source_config) >= max_files:
            raise ValueError(
                f"GitHub Repository Internal network / VPN source already has max_files={max_files} packages"
            )
    else:
        ...
        retained_doc_ids = {
            str((source_input.metadata or {}).get("doc_id") or "").strip() for source_input in retained_inputs
        }
        retained_doc_ids.discard("")
        if doc_id not in retained_doc_ids and len(retained_doc_ids) >= max_files:
            raise ValueError(
                "GitHub Repository Internal network / VPN source already has "
...
async def submit_teams_window_package(
    *,
    ...
    from memforge.local_agent.source_contract import TEAMS_TOMBSTONE_REASONS
    from memforge.local_agent.teams_contract import validate_teams_window_payload

    source_id = str(source["id"])
    payload_kind = validate_teams_window_payload(
        raw_payload,
    ...
    if (
        payload_kind == "scope_attestation"
        and str(raw_payload.get("collection_attempt_id") or "").strip() != str(collection_attempt_id or "").strip()
    ):
    ...
    package = {
        ...
        "last_modified": submitted_at,
        "space_or_project": str(
            raw_payload.get("team_name")
            or raw_payload.get("channel_name")
