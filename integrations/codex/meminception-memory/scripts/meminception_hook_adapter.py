...
MemInception Admin API calls and translates responses back into hook output.
...
DEFAULT_AGENT_QUEUE_DB = Path.home() / ".meminception-agent" / "queue.sqlite"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="MemInception agent hook adapter")
    parser.add_argument("mode", choices=("context", "submit-session", "worker-run-once"))
    parser.add_argument("--client", default=os.getenv("MEMINCEPTION_HOOK_CLIENT", "codex"))
    parser.add_argument("--api-url", default=os.getenv("MEMINCEPTION_API_URL", DEFAULT_API_URL))
    parser.add_argument("--timeout", type=float, default=_env_float("MEMINCEPTION_HOOK_TIMEOUT_SECONDS", DEFAULT_TIMEOUT_SECONDS))
    ...
    try:
        ...
        print(
            json.dumps({
                "continue": True,
                "systemMessage": f"MemInception hook skipped: {_safe_exception_message(exc)}",
            }),
        )
        return 0
...
def _run_context(payload: dict[str, Any], *, client: str, api_url: str, timeout: float) -> int:
    ...
    request = {
        ...
        "max_memories": _env_int("MEMINCEPTION_HOOK_MAX_MEMORIES", 5),
    }
...
def _agent_window_worker_command(*, api_url: str, timeout: float) -> list[str]:
    ...
    if script_path.exists() and script_path.name.startswith("meminception_hook"):
        command = [sys.executable, str(script_path)]
    else:
        command = [sys.executable, "-m", "meminception.hook_adapter"]
    return [
        *command,
        "worker-run-once",
        "--api-url",
...
def _agent_worker_timeout() -> float:
    return _env_float("MEMINCEPTION_AGENT_WORKER_TIMEOUT_SECONDS", DEFAULT_AGENT_WORKER_TIMEOUT_SECONDS)
...
def _post_json(path: str, payload: dict[str, Any], *, api_url: str, timeout: float) -> dict[str, Any]:
    ...
    api_token = os.getenv("MEMINCEPTION_API_TOKEN")
    if api_token:
...
def _session_window_payload(
    *,
    ...
    return {
        ...
        "plugin_version": os.getenv("MEMINCEPTION_PLUGIN_VERSION", PLUGIN_VERSION),
        "client": client,
        "session_id": session_id,
        "trigger": trigger,
        "workspace": workspace,
...
def _agent_queue_db_path(path: str | Path | None = None) -> Path:
    ...
    return Path(os.getenv("MEMINCEPTION_AGENT_QUEUE_DB") or DEFAULT_AGENT_QUEUE_DB).expanduser()
