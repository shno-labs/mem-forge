...
assert.match(setupSource, /<SettingsField[\s\S]*label="Initial history"/);
assert.match(setupSource, /<SettingsField[\s\S]*label="Rolling retention"/);
assert.match(setupSource, /PopoverPrimitive\.Trigger/);
assert.match(setupSource, /aria-label=\{`About \$\{label\}`\}/);
assert.match(setupSource, /className="mt-4 grid gap-5 sm:grid-cols-2 sm:gap-x-8"/);
assert.match(setupSource, /className="h-10 w-full data-size-default:h-10 sm:w-56"/);
assert.doesNotMatch(setupSource, /<select/);
