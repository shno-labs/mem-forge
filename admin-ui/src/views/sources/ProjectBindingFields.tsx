...
function ProjectPicker({
  ...
  return (
    <select
      ...
      disabled={loading}
      className="h-8 min-w-0 flex-1 rounded-md border border-input bg-background px-3 text-sm text-foreground shadow-none outline-none transition-colors focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring/40 disabled:cursor-not-allowed disabled:opacity-50"
    >
      ...
      {items.map((project) => (
        <option key={project.id} value={project.key}>
          ...
          {project.key === UNSORTED_PROJECT_KEY ? " (built-in)" : ""}
        </option>
      ))}
    </select>
  );
