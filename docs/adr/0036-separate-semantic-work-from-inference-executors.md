...
Accepted as target design on 2026-09-21; amended on 2026-09-22 to make classification model-neutral, remove confidence-based fallback, admit complete same-Unit pair classification, add Change Impact Classification, and separate binary Support semantics from `NEEDS_FULL`/`UNRESOLVED` execution control. Implementation is deferred.
...
4. Change Impact Classification is eligible judgment work. It receives one fixed claim and one capacity-safe `ChangeBundle`, and returns `AFFECTED` or `UNAFFECTED`. Multiple groups should share one bundle when they fit; multiple bundles are OR-reduced by code. `MODIFIED`, `REMOVED` and `AMBIGUOUS` Evidence bypass this classifier and enter Support Assessment; `UNKNOWN` is application-owned `UNRESOLVED(partial_coverage)` and KEEP.
...
6. Complete Support Assessment remains compound `GenerationWork`. `EvidenceFragment`, `ReadingGroup`, `AssessmentContext` and `AssessmentScope` are distinct. The deterministic planner prepares Delta contexts plus the remaining Full continuation, and starts Full when Delta already equals Full or Full is no more expensive than Delta plus continuation. Delta may finalize `SUPPORTED` or transition through internal `NEEDS_FULL`; only completed Full may finalize `UNSUPPORTED`. The final semantic result is therefore `SUPPORTED(primary_ref, required_refs[]) | UNSUPPORTED`, while incomplete coverage or execution is the separate application result `UNRESOLVED(reason)`. The planner sends old exact excerpt only for `MODIFIED`, `REMOVED` and `AMBIGUOUS`; `EXACT_UNCHANGED` and `CONTAINER_CHANGED` use current material, while `UNKNOWN` does not invoke the model.
...
11. Streamed Support output carries only current `witness_delta` additions. Application code validates and monotonically union-merges them into supporting/opposing witness sets; every carried ref is rehydrated with exact current text and Primary eligibility in the next call. The model cannot drop an earlier witness by omission and does not emit cumulative Support status. `NEEDS_FULL` and `UNRESOLVED` are execution control/results, not lifecycle states.
12. Support Planning/Assessment exclusively owns Delta-to-Full semantic coverage. `DestructiveValidation` verifies the completed authoritative Full receipt, manifests, witnesses, active-Support count and stale guards; it never runs a duplicate semantic scan.
