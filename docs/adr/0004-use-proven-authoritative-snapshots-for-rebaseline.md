...
An active Support Assertion is the lifecycle authority; `memory_sources` is its source/document provenance projection, not an independent deletion authority. A Lifecycle Plan may remove only the exact Evidence References owned by its Source and Source Unit. Removing one edge deletes the shared `(Memory, Source, document)` projection only when no other active Support resolves to that same triple, including a reincarnated Source Unit that reuses a document locator. A Memory becomes terminal only when neither active Support nor source projection remains. Active Support without its exact projection is corruption and makes the transaction fail closed and roll back. Cloud implementations lock the Memory and perform removal, invariant checks, terminal transition, Review invalidation, and vector-outbox publication in the same transaction.
