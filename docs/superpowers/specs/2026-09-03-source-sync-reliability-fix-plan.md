...
**资源评估收敛：** 优先消除可复现的多余工作：结构规划只解析 block 坐标，完整 inline/HTML 校验仍归 compiler；采集正文沿原有单文件接口及时上传/释放。密集结构解析是已有成本，不是两份 GitHub 文件读取错误的根因。通用解析预算系统不作为 B 的默认前置，不引入新的缓存、批次或任务状态；也不因局部优化就宣称任意大文件安全。B 的版本/字节正确性与既有输入范围的资源非回归仍需验证。
