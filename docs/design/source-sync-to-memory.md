...
3. 按 claim 汇集所有批次的发现，取回准确原文及可确定的原标题等祖先 Fragment。若汇总仍超限，逐层选择一组完整证明及必要条件，同时保留不同的潜在反例、例外和依赖；每条 finding 都须有保留或消除冗余的处置。相关的重复例子不都属于必要证明，不能用例子替代普遍规则本身的依据。旧 Evidence 的当前匹配也是可处置候选。摘要只作导航，不能成为 Evidence。
4. 最终请求可以合装多个 claim；联合考虑上述证明与不同挑战，返回固定 claim 的判断与一组完整当前 Evidence。完整全文扫描后，准备、装箱和发送共用省略纯历史解释的最终范围；delta 保留其历史上下文。归并是否继续按重建后的完整请求及输出预算判断，不能仅比较摘要长度。不存在“每批都没冲突，所以整体通过”的投票规则。
