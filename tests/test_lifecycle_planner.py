...
def test_an_add_restating_a_kept_old_memory_creates_its_own_memory() -> None:
    old = _memory()
    candidate = RawMemory(content=old.content, memory_type=old.memory_type, evidence_quote=old.content)
    plan = build_lifecycle_plan(
        plan_id="plan-restated",
        scope=_scope(),
        gate_state=LifecycleGateState.ENABLED,
        operations=(
            ReconcileOperation(action=ReconcileAction.ADD, memory=candidate),
            ReconcileOperation(action=ReconcileAction.NOOP, memory_id=old.id),
        ),
        incumbents={old.id: old},
        source_support_unit_ids={old.id: ("eu-old",)},
        all_active_support_unit_ids={old.id: ("eu-old",)},
    ...
    [created] = [item for item in plan.mutations if item.mutation_type is LifecycleMutationType.CREATE_MEMORY]
    assert created.memory_id != old.id
    assert [(item.mutation_type, item.memory_id, item.evidence_unit_ids) for item in plan.mutations] == [
        (LifecycleMutationType.CREATE_MEMORY, created.memory_id, ()),
        (LifecycleMutationType.ATTACH_SUPPORT, created.memory_id, ("eu-new",)),
    ]
    assert [request.memory_id for request in plan.relation_discovery_requests] == [created.memory_id]


def test_plan_rejects_a_support_attachment_outside_its_incumbents_and_creations() -> None:
    plan = _build(gate=LifecycleGateState.ENABLED)
    outside = LifecycleMutation(
        LifecycleMutationType.ATTACH_SUPPORT,
        memory_id="mem-other-unit",
        source_id=plan.scope.source_id,
        evidence_unit_ids=("eu-new",),
    )

    with pytest.raises(ValueError, match="outside this Plan's incumbents and creations"):
        replace(plan, mutations=(*plan.mutations, outside)).validate()
