...
async def test_gated_review_retains_last_successful_support_plan(db):
    ...
    common = dict(
        ...
        defaults=NewMemoryDefaults(
            ...
            doc_id="doc-1", source_type="github_repo", access_context_hash=access_hash,
        ),
    )
    for plan_id, action in (("verified-plan", ReconcileAction.NOOP), ("review-plan", ReconcileAction.DELETE)):
        ...
        plan = build_lifecycle_plan(
            ...
            scope=ReconciliationScope(
                id=plan_id, source_id=source_id, source_unit_id="unit-1",
                base_unit_revision_id="unitrev-1", target_unit_revision_id="unitrev-1",
            ),
            gate_state=(await db.get_lifecycle_gate(source_id)).state,
            operations=(ReconcileOperation(
                action=action, memory_id=memory_id,
                memory=RawMemory(content=memory.content, memory_type=memory.memory_type) if action is ReconcileAction.NOOP else None,
