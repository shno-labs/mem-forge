...
class FailingLifecycleOutboxMemoryStore:
    ...
    async def attempt_lifecycle_vector_delivery(self, *args, **kwargs):
        from memforge.memory.lifecycle_plan import (
...
class ProjectionFragmentCandidateExtractor(ProjectionFragmentRecordingExtractor):
    async def extract_projection_fragment_memories(self, catalog, **kwargs):
        del kwargs
        self.fragment_calls.append(catalog)
        index = len(self.fragment_calls) - 1
        primary = next(fragment for fragment in catalog.fragments if fragment.primary_eligible)
        selection = catalog.resolve_selection(primary_ref=primary.reference)
        return MemoryExtractionResult(
            memories=[
                RawMemory(
                    content=f"Durable claim from projection fragments {index}.",
                    memory_type="fact",
...
def _valid_png_bytes() -> bytes:
    output = BytesIO()
    Image.new("RGB", (1, 1), color=(20, 40, 60)).save(
        output,
        format="PNG",
    )
        ...
        return RawContent(
            ...
            artifacts=(
                RawSourceArtifact(
                    ...
                    declared_size_bytes=len(_valid_png_bytes()),
...
async def test_artifact_summary_commits_with_the_same_projected_revision(db: Database) -> None:
    ...
    extractor = ProjectionFragmentArtifactSummaryExtractor()
    orchestrator = GeneSyncOrchestrator(
        db=db,
        doc_store=StubDocumentStore(),
        memory_extractor=extractor,
...
async def test_projection_images_load_only_after_shared_multimodal_admission(
    ...
    await asyncio.wait_for(extractor.target_reached.wait(), timeout=2)
    admitted_read_count = stores[0].artifact_read_count
    assert admitted_read_count >= 1
    ...
    assert [state.last_sync_status for state in states] == ["success", "success"]
    assert stores[1].artifact_read_count == admitted_read_count
...
async def test_failed_newer_provider_target_still_supersedes_recovered_intent(
    ...
    superseded = attempts[attempt.id]
...
async def test_sync_supersedes_completed_derivation_when_its_lifecycle_plan_is_applied(
    ...
    orchestrator = GeneSyncOrchestrator(
        db=db,
        doc_store=StubDocumentStore(),
        memory_extractor=ProjectionFragmentCandidateExtractor(),
        memory_engine=memory_engine,
        memory_store=None,
        max_concurrent=1,
    )
...
@pytest.mark.asyncio
async def test_source_sync_worker_drains_vector_outbox_without_a_source_run(
    db: Database,
    monkeypatch,
) -> None:
    import memforge.runtime as runtime
    ...
    class MemoryStore:
        ...
        async def attempt_lifecycle_vector_delivery(self):
            self.calls += 1
            return Delivery()
    ...
    async def pending_vector_task(**kwargs):
        assert kwargs["limit"] == 1
        return [object()]

    monkeypatch.setattr(db, "list_ready_lifecycle_vector_tasks", pending_vector_task)
    worker = runtime.SourceSyncWorker(
        db,
        AppConfig(),
        runtime_provider=provider,
        worker_id="worker-vector-outbox",
    )

    processed = await worker.process_lifecycle_vector_delivery_once()

    assert processed
    assert provider.runtime_builds == 1
...
async def test_scope_reentry_reextracts_exact_revision_without_reusing_retired_memory(
    ...
    class OneClaimExtractor(ProjectionFragmentRecordingExtractor):
        def __init__(self) -> None:
            super().__init__()
        ...
        async def extract_projection_fragment_memories(self, catalog, **kwargs):
            del kwargs
            self.claim_calls.append(catalog.target_unit_revision_id)
            [fragment] = [item for item in catalog.fragments if "Keep A7." in item.presentation_text]
            return MemoryExtractionResult(
                memories=[
                    RawMemory(
                        content="A7 remains enabled.",
...
@pytest.mark.asyncio
async def test_lifecycle_failure_never_attempts_document_snapshot_restore(
    db: Database,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    async def reject_snapshot_restore(*_args, **_kwargs) -> None:
        ...
        restore_calls += 1
        raise AssertionError("atomic lifecycle must not require snapshot restore")
...
async def test_extraction_failure_never_requires_document_snapshot_restore(
    ...
    async def reject_restore(*args, **kwargs) -> None:
        del args, kwargs
        nonlocal restore_calls
        restore_calls += 1
        raise AssertionError("atomic lifecycle must not require snapshot restore")

    monkeypatch.setattr(db, "restore_document_snapshot", reject_restore)
    extractor = FailingMemoryExtractor()
    state = await GeneSyncOrchestrator(
        db=db,
        doc_store=StubDocumentStore(),
...
@pytest.mark.asyncio
async def test_unchanged_document_survives_pending_lifecycle_vector_delivery(
    db: Database,
) -> None:
    source_id = "src-unchanged-lifecycle-vector"
    markdown = "# Jira 0\n\nBody"
    await _insert_document_with_metadata(
        db,
        source_id=source_id,
