...
@pytest.mark.asyncio
async def test_any_active_v2_support_makes_legacy_memory_already_supported() -> None:
    candidate = replace(
        _candidate(),
        reason_codes=("unit_revision_lineage_invalid",),
        active_v2_unit_ids=("independent-v2-unit",),
    )
    database = _RecoveryFakeDb(candidate)
    ...
    assert prepared.report.entries[0].disposition is (
