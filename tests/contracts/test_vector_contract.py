...
class VectorStoreContract:
    ...
    def test_declares_a_distance_metric(
        self, adapters: ContractAdapters
    ) -> None:
    ...
    def test_similarity_is_floored_at_zero(
        self, adapters: ContractAdapters
    ) -> None:
    ...
    async def test_upsert_then_get_record_round_trips_metadata(
        self, adapters: ContractAdapters
    ) -> None:
        memory = make_memory("m1")
        await adapters.vector.upsert(
            ["m1"], [list(DEFAULT_EMBEDDING)], [make_vector_metadata(memory)]
        )
    ...
    async def test_get_record_returns_none_for_missing_id(
        self, adapters: ContractAdapters
    ) -> None:
    ...
    async def test_query_hides_other_users_private_rows(
        self, adapters: ContractAdapters
    ) -> None:
        ...
        hits = await adapters.vector.query(
            list(DEFAULT_EMBEDDING), scope, None, limit=10
        )
        assert {mid for mid, _ in hits} == {"owner-private"}

    async def test_query_drops_private_rows_when_scope_excludes_them(
        self, adapters: ContractAdapters
    ) -> None:
        ...
        hits = await adapters.vector.query(
            list(DEFAULT_EMBEDDING), make_scope(), None, limit=10
        )
